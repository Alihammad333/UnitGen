...
function selectFilesForTesting(input) {
  /*
    UnitGen full-package scan mode.

    Important:
    - Do not prefer package.json main/module/browser/exports.
    - Do not reduce npm packages to entry files.
    - Always use the complete file list already collected by resolveInput().
    - resolveInput() already calls collectJsFiles(), so folder/package input.files
      already contains all .js/.mjs/.cjs files found recursively.
  */
  return {
    mode: "standard",
    files: input.files,
...
if (isUnrecoverableJestResult(initialResults)) {
  ...
  if (
    ...
    if (includeDynamicBehaviorContexts) {
      console.log(
        `ℹ️ Surviving package behavior coverage is small; preparing up to ${dynamicBehaviorContextLimit} dynamic API behavior context(s).`
      );
    }
