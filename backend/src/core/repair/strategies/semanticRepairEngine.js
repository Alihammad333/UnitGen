    ...
    CallExpression(path) {
      ...
      testFnPath.traverse({
        CallExpression(innerPath) {
          ...
          /* ==========================================
             1. FIX rejects misuse on non-promise values
          ========================================== */
          if (runtimeFlags.isRejectsMatcherMisuse && modifier === "rejects") {
            ...
            if (receivedLiteral) {
              const valueNode = buildAssertionValueNode(receivedLiteral);
              if (valueNode) {
                innerPath.replaceWith(
                  t.callExpression(
                    t.memberExpression(
                      t.callExpression(t.identifier("expect"), [valueNode]),
                      t.identifier("toBe")
                    ),
                    [valueNode]
                  )
          ...
          /* ==========================================
             2. FIX resolves misuse on non-promise values
          ========================================== */
          if (runtimeFlags.isResolvesMatcherMisuse && modifier === "resolves") {
            if (recoveredCall?.kind === "async-call") {
              innerPath.replaceWith(
                buildResolvesDefinedAssertion(recoveredCall.callNode)
              );
              changed = true;
              innerPath.stop();
              return;
          ...
          /* ==========================================
             3. FIX toThrow misuse against direct values
          ========================================== */
          if (runtimeFlags.isToThrowMatcherMisuse && matcherName === "toThrow") {
            if (recoveredCall?.kind === "sync-call") {
              innerPath.replaceWith(
                buildSyncThrowAssertion(recoveredCall.callNode, errorText)
              );
          ...
          if (failureType === "ASSERTION" || runtimeFlags.isAssertionError) {
            ...
            // numeric oracle mismatch -> stable numeric shape
            if (
              matcherName === "toBeCloseTo" &&
              runtimeFlags.isNumericOracleMismatch
            ) {
              if (
                replaceExpressionWithMultipleStatements(
                  innerPath,
                  buildNumberShapeAssertions(expectArgPath.node)
                )
              ) {
                changed = true;
                innerPath.stop();
            ...
            // null/undefined oracle mismatch -> not nullish
            if (
              ...
              if (
                replaceExpressionWithMultipleStatements(
                  innerPath,
                  buildNotNullishAssertions(expectArgPath.node)
                )
              ) {
                changed = true;
                innerPath.stop();
            ...
            // deep equality mismatch with literal object/array -> shape assertion
            if (
              (matcherName === "toEqual" || matcherName === "toStrictEqual") &&
              expectedArg &&
              runtimeFlags.isDeepEqualityMismatch
            ) {
              const shape = shouldUseShapeForExpectedNode(expectedArg);

              if (
                shape === "array" &&
                replaceExpressionWithMultipleStatements(
                  innerPath,
                  buildArrayShapeAssertions(expectArgPath.node)
                )
              ) {
                changed = true;
                innerPath.stop();
              ...
              if (
                ...
                replaceExpressionWithMultipleStatements(
                  innerPath,
                  buildObjectShapeAssertions(expectArgPath.node)
                )
              ) {
                changed = true;
                innerPath.stop();
          ...
          /* ==========================================
             6. Reference error lightweight fallback
          ========================================== */
          if (
            ...
            // Avoid inventing bindings for test DSL/global names.
            if (
              [
                "expect",
                "test",
            ...
            const binding = innerPath.scope.getBinding(varName);

            if (!binding && varName !== "undefined") {
              try {
                innerPath.scope.push({
                  id: t.identifier(varName),
