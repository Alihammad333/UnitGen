  ...
  try {
    ...
    traverse(ast, {
      ImportSpecifier(path) {
        if (path.node.local?.name) allowed.add(path.node.local.name);
      },
      ...
      FunctionDeclaration(path) {
        if (path.node.id?.name) allowed.add(path.node.id.name);
      },
    ...
    // Ignore. Fallback allowed identifiers are added separately.
  }
...
function sanitizeRepairCases({
  ...
  for (const rawCase of cases) {
    ...
    if (!hasResultCenteredAssertion(c.assert)) continue;
    ...
    if (hasMissingRequiredArguments({ fnName, act: c.act, params })) {
      continue;
    }
...
function getFileIdentifierSet(code) {
  ...
  try {
    ...
    traverse(ast, {
      ImportSpecifier(path) {
        if (path.node.local?.name) ids.add(path.node.local.name);
      },
      ImportDefaultSpecifier(path) {
        if (path.node.local?.name) ids.add(path.node.local.name);
      },
      ...
      FunctionDeclaration(path) {
        if (path.node.id?.name) ids.add(path.node.id.name);
      },
      ClassDeclaration(path) {
        if (path.node.id?.name) ids.add(path.node.id.name);
      },
    ...
    // Ignore. Syntax validation happens elsewhere.
  }
...
export async function attemptLLMRepair({
  ...
  try {
    ...
    const ctx = Array.isArray(contexts)
      ? contexts.find((c) => c.testFilePath === filePath)
      : null;

    if (!ctx) {
      console.log("⚠️ Context not found for repair.");
      return null;
    }
    ...
    console.log("✅ LLM repair applied.");
    return repairedCode;
  } catch (err) {
