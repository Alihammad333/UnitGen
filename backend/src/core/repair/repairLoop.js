...
====================================================== */
function recordFailureHistory(filePath, newFailures, attempt) {
  const relevant = newFailures.filter((f) => f.filePath === filePath);
...
async function tryLocalRepair({
  ...
  const originalCode = fs.readFileSync(filePath, "utf8");
  const fileFailures = failures.filter((f) => f.filePath === filePath);
  ...
  for (const failure of fileFailures) {
    emitEvent("failure_classified", {
      ...
      repairStrategy: "AST/Semantic Repair",
      status: "classified",
    });
  }
  ...
  for (const failure of fileFailures) {
    ...
    if (astResult && astResult !== originalCode) {
      repairedCode = astResult;
      console.log("🧠 AST Repair Applied");
      break;
    }
  }

  if (!repairedCode) {
    console.log("➡️ AST skipped");
  }
  ...
  if (!repairedCode) {
    for (const failure of fileFailures) {
      ...
      if (semanticResult && semanticResult !== originalCode) {
        repairedCode = semanticResult;
        console.log("🧠 Semantic Repair Applied");
        break;
      }
    }

    if (!repairedCode) {
      console.log("➡️ Semantic skipped");
    }
  }
  ...
  if (!isRunnableTest(repairedCode)) {
    console.log("⚠️ Invalid AST/Semantic output.");
    return {
      accepted: false,
      attempted: true,
      failures,
  ...
  if (hasUndeclaredTestReferences(repairedCode)) {
    console.log("⚠️ Local repair introduced undeclared references.");
    return {
      accepted: false,
      attempted: true,
      failures,
  ...
  const backup = originalCode;

  try {
    saveRepairedFile(filePath, repairedCode);

    const result = await runJest({ configPath });

    if (!isValidTestRun(result)) {
      console.log("⚠️ Local repair produced invalid test run. Reverting.");
      saveRepairedFile(filePath, backup);
      return {
        accepted: false,
        attempted: true,
        failures,
    ...
    if (hasRegression(lastResult, result)) {
    ...
    const newFailures = extractFailures(getJestJson(result));
      ...
      if (
        ...
        emitEvent("repair_candidate_repaired", {
          ...
          repairStrategy: "AST/Semantic Repair",
          after: "Repair candidate marker removed or candidate repaired",
          status: "repaired",
        });
      }
    ...
    console.log("⚠️ Local repair was not an improvement. Reverting.");

    emitEvent("repair_rejected", {
      testFile: path.basename(filePath),
      failureType: "Local repair",
      repairStrategy: "AST/Semantic Repair",
      after: "Repair rejected because it was not an improvement",
      status: "rejected",
    });
...
async function tryLLMRepair({
  ...
  console.log(`🤖 LLM repairing: ${path.basename(filePath)}`);
  ...
  if (!isRunnableTest(repairedCode)) {
    console.log("⚠️ LLM produced invalid or non-runnable test.");
    return {
      accepted: false,
      attempted: true,
      failures,
  ...
  if (hasUndeclaredTestReferences(repairedCode)) {
    console.log("⚠️ LLM repair introduced undeclared references.");
    return {
      accepted: false,
      attempted: true,
      failures,
  ...
  try {
    ...
    if (!isValidTestRun(result)) {
      console.log("⚠️ LLM repair produced invalid test run. Reverting.");
      saveRepairedFile(filePath, backup);
      return {
        accepted: false,
        attempted: true,
    ...
    console.log("⚠️ LLM repair was not an improvement. Reverting.");
    saveRepairedFile(filePath, backup);
