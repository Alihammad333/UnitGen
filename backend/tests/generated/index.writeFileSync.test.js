...
beforeAll(async () => {
  ...
  writeFileSync = __unitgenResolveExport(mod, "writeFileSync", false);
});
...
describe("writeFileSync", () => {
  test("auto-generated (prototype)", async () => {
    ...
    if (typeof writeFileSync !== "function") {
      throw new TypeError("writeFileSync import did not resolve to a function");
    }
    ...
    let result;
    try {
      result = writeFileSync(file, obj, options);
      if (result && typeof result.then === "function") {
        result = await result;
      }
      expect(fs.existsSync(__unitgenFilePath)).toBe(true);
    } catch (error) {
  ...
  test("fallback checks source-aware result shape", () => {
    {
      ...
      const file = __unitgenFilePath;
      const obj = { name: "unitgen", ok: true };
      const options = {};
      const result = writeFileSync(file, obj, options);
